(module r5rs mzscheme
                   ...
                   (dw (dws_1 ...) 
                       (in-hole ec_1
                                ((mark #%dynamic-wind)
                                 ...
                                 (mark cp_3))))))
      . --> . 
      (term-let ([x_1 (variable-not-in (term prog) 'd)]
                 [x_2 (variable-not-in (term prog) 'x)])
        (term 
         (store ((ptr_s sv_s) ...)
           (dw (dws_1 ...)
               (in-hole ec_1
  ...
  (define Helper--functions
    (list
     ...
     ((store ((ptr_s sv_s) ...) (dw (dws_1 ...) (in-hole ec_1 (throw x_k dws_2 ... (in-hole ec_2 e_2)))))
      . --> .
      (term (store ((ptr_s sv_s) ...) (dw (dws_2 ...)
                                          (begin ,@(trim (term (dws_2 ...)) (term (dws_1 ...)))
                                                 (in-hole ec_2 e_2))))))))

  (define (trim stack1 stack2)
    (cond
      [(equal? (first-symbol* stack1) (first-symbol* stack2))
       (trim (cdr stack1) (cdr stack2))]
      [else
       (match (list stack1 stack2)
         [`(((,x1 ,pre1 ,post1) ...) ((,x2 ,pre2 ,post2) ...))
          (append post1 (reverse pre2))])]))
  ...
  (define Multiple--values--and--call-with-values
    (list
     ...
     ((in-named-hole single
      ...
      . --> .
      (term (error "wrong number of values")))
  ...
  (define reductions (append Basic--syntactic--forms1
                             Basic--syntactic--forms2
                             List--constructors
                             List--accessors
                             List--predicates
                             Cons--cell--mutation
